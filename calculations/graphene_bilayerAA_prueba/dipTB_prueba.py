...
ggr=grid.UniformCartesianGrid(2,[[-0.75/a,0.75/a],[-0.75/a,0.75/a]],[500,500], origin=(0.0,0), filter=filter, filter_args= filter_args)

print(f'vprint in line: 33 --> {0.75/a=}')
...
#gcr=cr.crystal.from_W90_TB_file(filename='/home/lplaja/crystal_project/dip_prueba/Wannier90 data/gr_tb.dat', 
gcr=cr.crystal.from_W90_TB_file(filename='/home/lplaja/crystal_project/dip_prueba/Wannier90 data/grAA_tb.dat', 
                                grid=ggr, species_name='graphene', threshold_hopping=0.01)
...
Efield=polarizedHarmonicElectricField(tt,I_W__cm2=5e10,lambda0_nm=lambda0*1e9, env=env_sin2, env_parameters={'start': 0, 'end': 1, 'ton': 0.5, 'toff': 0.5},
                                      phi_rad=-np.pi/2, chi_rad=np.pi/2,ellip=0)

print(f'vprint in line: 46 --> {T0=}')
print(f'vprint in line: 46 --> {lambda0=}')
print(f'vprint in line: 46 --> phi_rad={-np.pi/2}')
print(f'vprint in line: 46 --> chi_rad={np.pi/2}')
